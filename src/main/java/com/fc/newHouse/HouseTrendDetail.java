...
//1 楼盘动态详情页接口（PC/WAP）
public class HouseTrendDetail extends TestBase {
    ...
    public void setUp(){
        ...
        testCaseExecel = System.getProperty("user.dir")+prop.getProperty("HouseTrendDetail");
    }
    @DataProvider(name = "postData")
    public Object[][] post() throws IOException{
        return  dtt(testCaseExecel,0);
    }
    @Test(dataProvider = "postData" ,description = "新房：楼盘动态详情页接口（PC/WAP）")
    public void filterCon(String contenx)throws Exception{

        CloseableHttpClient httpClient = HttpClients.createDefault();
        HttpPost post = new HttpPost("https://dict.izhiliao.com/house/api/trendDetail");
            ...
            if (res.getStatusLine().getStatusCode() == HttpStatus.SC_OK){
               ...
               JSONObject jsonObject =JSONObject.parseObject(result);
                ...
                if (msgValue.equals("操作成功")){
                    ...
                    String trendName = jsonObject1.getString("trendTitle");
                    String expectTrendName="北京半岛";
                    if (trendName.contains(expectTrendName)){
                        System.out.println("该楼盘包含："+expectTrendName);
